  ...
  const handleLogin = async()=>{
    try {
      const res = await axios.post(`${URL}/user/login`, { email, password }, {
        withCredentials: true, // This is required to include credentials
        headers: {
  ...
  return (
    ...
    <div className="w-full flex justify-center items-center h-[70vh]">
      <div className="flex flex-col justify-center items-center space-y-4 w-80% md:w-[25%]">
        ...
        <input onKeyDown={handleKeyDowm} onChange={(e)=>setEmail(e.target.value)} className="w-full px-4 py-2 border-2 border-black outline-0" type="text" placeholder="Enter your email" />
        <input onKeyDown={handleKeyDowm} onChange={(e)=>setPassword(e.target.value)} className="w-full px-4 py-2 border-2 border-black outline-0" type="password" placeholder="Enter your password" />
